...
export const RoomBrowser = ({ categoryId, categoryName }: Props) => {
  ...
  // categoryId 변경 시 렌더 단계에서 즉시 초기화 — useEffect 내 setState는 cascading 렌더 유발
  ...
  return (
    <div
      className={`${styles.container} ${isPlaceholderData ? styles.containerLoading : ""}`}
    >
      {rooms.length === 0 && !debouncedQuery ? (
        <>
          ...
          <div className={styles.emptyChannel}>
            ...
            <p className={styles.emptyChannelText}>
              해당 채널에서 채팅방을 만들어보세요
            </p>
            {canManage && (
                    ...
                    <div key={room.id} className={styles.cardWrap}>
                      ...
                      >
                        ...
                        <div className={styles.cardFooter}>
                          ...
                          {isFull && (
                            <span className={styles.fullBadge}>꽉 참</span>
                          )}
                        </div>
                      </button>
                      {isAdmin && (
      ...
      {deleteTarget && (
        ...
        >
          ...
          >
            ...
            <p className={styles.fullModalDesc}>
              <strong>{deleteTarget.title}</strong> 채팅방이 삭제됩니다.
              <br />이 작업은 되돌릴 수 없습니다.
            </p>
