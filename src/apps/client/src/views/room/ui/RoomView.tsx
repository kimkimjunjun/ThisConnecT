...
const NICKNAME_POOL = [
  '하늘별', '바람소리', '달빛강물', '별빛구름', '초록나무',
  '파란하늘', '노란해살', '빨간꽃잎', '보라비', '하얀눈',
]

const INITIAL_MESSAGES: ChatMessage[] = [
  { id: 'im1', senderId: 'p0', senderName: '하늘별', text: '안녕하세요! 처음 들어왔어요 😊', time: '오후 2:31' },
  { id: 'im2', senderId: 'p1', senderName: '바람소리', text: '어서 오세요~', time: '오후 2:32' },
  { id: 'im3', senderId: 'p0', senderName: '하늘별', text: '오늘 날씨가 정말 좋네요', time: '오후 2:33' },
  { id: 'im4', senderId: 'p2', senderName: '달빛강물', text: '그러게요 ㅋㅋ 드라이브 가고 싶다', time: '오후 2:35' },
  { id: 'im5', senderId: 'p1', senderName: '바람소리', text: '저도요! 누가 태워줘요', time: '오후 2:36' },
]

const buildParticipants = (room: ChatRoom, myNickname: string | null): Participant[] => {
  const count = Math.max(1, room.currentCount)
  const list: Participant[] = [{ id: 'me', nickname: myNickname ?? '나', isMe: true }]
...
export const RoomView = ({ room, categoryId }: Props) => {
  ...
  const participants = useMemo(() => buildParticipants(room, nickname), [room, nickname])
  ...
  const handleSend = useCallback(() => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: `m-${Date.now()}`, senderId: 'me', senderName: nickname ?? '나', text, time: timeStr },
    ])
    setInputText('')
  }, [inputText, nickname])
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.participantsSection}>
        <span className={styles.sectionLabel}>참여자 · {participants.length}명</span>
      ...
      <div className={styles.audioControls}>
        ...
        >
          <span className={styles.audioBtnIcon}>{isSpeakerOn ? '🔊' : '🔈'}</span>
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={styles.messageItem}>
              <div className={`${styles.messageAvatar} ${isMe ? styles.messageAvatarMe : ''}`}>
